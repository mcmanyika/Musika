...
interface TransportListProps {
  ...
  onViewBids: (deal: BuyerOrder) => void;
  searchQuery: string;
...
const DealItem: React.FC<{ 
  ...
  currentUserId?: string;
  transactions?: any[];
}> = ({ deal, bids, onPlaceBid, onViewBids, yields, userRatingStatsMap, onViewTransporter, onAcceptBid, currentUserId, transactions }) => {
...
const TransportList: React.FC<TransportListProps> = ({ deals, bids, onPlaceBid, onViewBids, searchQuery, yields, userRatingStatsMap, onViewTransporter, onAcceptBid, currentUserId, transactions }) => {
  if (deals.length === 0) {
    return (
